...
- **AI calls:** `src/lib/ai.ts` calls a Supabase edge function `openai-proxy` (model `gpt-5.6-luna`, `reasoning_effort: "none"`). Pattern: build prompt, `openaiChat()` with a strict JSON schema, use the parsed result directly. Copy this pattern for any new AI function.
  > **Superseded September 7, 2026.** This previously read `src/lib/groq.ts` / `groq-proxy` / `llama-3.3-70b-versatile`, using a regex to extract JSON from free text. Groq retired that model and the whole AI layer was migrated to OpenAI (commit `1bf14dd`); `src/lib/groq.ts` no longer exists. The old `groq-proxy` function and `GROQ_API_KEY` secret are still deployed as a rollback path, but are not the live path. References to `groq.ts` elsewhere in this plan are historical records of completed phases, not instructions.
